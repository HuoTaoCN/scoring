...
const TODAY_STR = today.toLocaleDateString('zh-CN');
...
const DATE_MINUS_3 = getDateBefore(3);
const DATE_MINUS_5 = getDateBefore(5);
const DATE_MINUS_7 = getDateBefore(7);
...
export const MOCK_CASES: MockCase[] = [
  {
    ...
    input: {
      ...
      reply_text: `您好！您反映的幸福家园小区南门路灯损坏问题已收悉。我局市政设施管理处已于${DATE_MINUS_3}上午派出维修人员前往现场核实。经查，该处确有两盏路灯因线路老化导致故障。维修人员已现场更换了老化线路和灯泡，目前路灯已恢复正常照明。感谢您对城市管理工作的支持与监督！`,
      metadata: {
        category: "求助",
        duration_days: 1,
        ticket_id: "20250101-001",
        timestamp: `${DATE_MINUS_3} 09:30:00`,
    ...
    result: {
      ...
      suggested_reply: `您好！\n**一、收到诉求情况**\n您于${DATE_MINUS_3}通过电话反映的关于“幸福家园小区南门路灯损坏维修诉求”的问题，我单位已收悉。感谢您的信任与支持！\n**二、调查核实情况**\n我们立即安排市政设施管理处开展调查：\n• 调查方式：现场核查\n• 核实事实：经查，该处确有两盏路灯因线路老化导致故障，影响市民夜间出行。\n• 政策依据：《城市照明管理规定》第十九条\n**三、处理意见与答复**\n经研究，现答复如下：\n• 维修人员已现场更换了老化线路和灯泡，并对周边路灯进行了排查。\n• 目前，路灯已恢复正常照明。\n**四、后续说明**\n• 我们将加强日常巡检，确保照明设施完好。\n• 联系人：赵工 电话：12319\n再次感谢您的理解与监督！\n（市市政管理处）\n${TODAY_STR}`,
  ...
  },
  {
    ...
    input: {
      ...
      metadata: {
        ...
        timestamp: `${DATE_MINUS_5} 14:00:00`,
  ...
  },
  {
    ...
    input: {
      ...
      metadata: {
        ...
        timestamp: `${DATE_MINUS_3} 10:00:00`,
  ...
  },
  {
    ...
    input: {
      ...
      metadata: {
        ...
        timestamp: `${DATE_MINUS_7} 15:30:00`,
  ...
  },
  {
    ...
    input: {
      ...
      metadata: {
        ...
        timestamp: `${DATE_MINUS_3} 09:00:00`,
